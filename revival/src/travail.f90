subroutine travail(E0in,title,pulsetype,wir,phase,le0wattcm2,tc,te,tf,iE0,logfile,t,ntps,ep,npos,v,x,id,dt,nt,xmu12,pot,delr,chi1in,chi2in,massreduite,pbfin,TVIB)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
real(8) :: t(nt),delt,t0,pi,omega,  norme,norme1,norme2,periode,delta,vp1(npos),vp2(npos),sigma,tmax,kmoyen(nt),rmoyen(nt),rmoyenlie(nt),rclapet1(nt),rclapet2(nt),f0
real(8) :: dissprob
real(8) :: dispers(nt)
...
dtper=dt/1024.d0
      ...
      do while(timeper.lt.tf)
         ...
         call calc_pulse(pulsetemp,timeper,tc,te,E0)
         call calc_champ(champ2,pulsetemp,wir,timeper,tc,phase)
         call calc_pulse(pulsetemp,timeper-dtper,tc,te,E0)
         call calc_champ(champ1,pulsetemp,wir,timeper-dtper,tc,phase)

         call airesint (int1tf, int2tf, int3tf, dtper, champ1, champ2)
      end do
      call calc_pulse(pulsetemp,timeper,tc,te,E0)
      call calc_champ(champ1,pulsetemp,wir,timeper,tc,phase)
      call calc_pulse(pulsetemp,tf,tc,te,E0)
      call calc_champ(champ2,pulsetemp,wir,tf,tc,phase)
...
write(*,*) "int1tf", int1tf
 ...
 do i=1,nt
    if (mod(i,100).eq.0) then
      ...
      call calc_champ(champ1,pulsetemp,wir,timeper,tc,phase)
      call calc_pulse(pulsetemp,tf,tc,te,E0)
      call calc_champ(champ2,pulsetemp,wir,t(i),tc,phase)
...
!       end do
!    end if
    call calc_pulse(pulset(i),t(i),tc,te,E0)

    call calc_champ(champ(i),pulset(i),wir,t(i),tc,phase)
