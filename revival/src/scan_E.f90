program scanE
    ...
    namelist /iofile/ title, pulsetype,E0min,E0max,dE0,ntl,phase,le0wattcm2,dt,npos,TVIB,cas,tsync,xmax
    read(5,iofile)
...
!read(*,*) test
 ...
 call pot_spec(pot(1,:),pot(2,:),xmu12, npos,delr,xmin) !construction des 2 potentiels de H2+ et du moment dipolaire
    ...
    do while ((iE0+(rank)).le.nE0)  
        ...
        call travail(E0,title,pulsetype,wir,phase,le0wattcm2,tc,te,tf,iE0+rank,logfile,t,nt,ep,npos,v,x,id,dt,nt,xmu12,pot,delr,chi1,chi2,massreduite,pbfin(iE0+rank),TVIB)
